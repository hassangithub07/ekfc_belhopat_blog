    ...
    return async function (req, res, next) {
        if (req.headers?.authorization || req.cookies?.access_token) {
            ...
            try {
                ...
                const user = await User.findById(decoded._id).select('first_name last_name image_url user_name country_code phone_number full_number is_private_account _id type color_code role_id').exec();
...
async function verifySocketToken(encoded_token) {
    if (encoded_token) {
        ...
        try {
            ...
            const user = await User.findById(decoded._id).select('first_name last_name user_name image_url _id type country_code phone_number full_number is_private_account full_number color_code role_id').exec();
            if (Date.now() >= decoded.exp * 1000) {
                throw new Error('Authentication error')
            }
            return user;
